...
const held: ConsentRequired = {
  ...
  boxes: { policy_acceptance: true, marketing_consent: true, networking_consent: true },
};
...
test("the boxes the API asked for are the boxes that come back", () => {
  ...
  const requiredOnly: ConsentRequired = {
    ...held,
    boxes: { policy_acceptance: true, marketing_consent: false, networking_consent: false },
  };
...
test("a cookie that does not claim the required box is not a consent step", () => {
  // The consent-required outcome exists precisely because Policy Acceptance is
  // outstanding, so a cookie saying otherwise is not one. Rendering it would put
  // a submit button on screen that nothing could ever enable.
