/**
 * Which language a Storefront page is served in, and how that is spelled in a
 * URL.
 *
 * Two vocabularies meet here and must not be confused. The URL carries a short
 * token — "en", "es" — because it is read by people, typed into chats and
 * printed on posters. Intl carries a language-and-region tag — "en-US",
 * "es-EC" — because number marks, month names and the position of a currency
 * symbol are regional facts, not language ones. This module owns the mapping
 * between them, so no page has to guess which of the two it is holding.
 *
 ...
 * Those arguments are passed separately in lib/format.ts and no locale may
 * reach them.
 *
 ...
 *
 * Both are always prefixed. There is no unprefixed default: "/" is not English,
 * it is an address with no language in it, and it redirects to one. The
 * alternative — English at "/" and Spanish at "/es" — makes one language the
 * unmarked case and leaves every English URL unable to say so, which is exactly
 * what breaks a shared link.
 */
 ...
 *
 * It is written by the language switcher and by nothing else. Neither this
 * middleware nor next-intl writes it as a side effect of a visit, because a
 * cookie set from a browser's Accept-Language would make a visitor's first
 * accidental landing stick for six months.
 *
 * The name is the one next-intl and Next's own i18n conventions use, so a
 * cookie set here is understood by anything else that reads one.
 */
...
export function resolveLocale(input: {
  ...
  // Only a token this Storefront serves counts. The cookie is ours, written by
  // the switcher alone, so anything else in it is stale or hand-made and is
 ...
 * names none.
 *
 * Region is dropped on the way in: "es-EC", "es-419" and "es" are all this
 * Storefront's Spanish, because there is only one Spanish to serve. Entries are
