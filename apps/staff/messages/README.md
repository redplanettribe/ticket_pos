...
| `outstandingAnswers` | An Event's **Outstanding Answers** (#313): which Tickets still owe required Ticket Questions an Answer. Its own namespace and not part of `ticketAnswers`, because that surface is one sale's Answers while this one is the whole Event's debt — one is opened from a sales row, the other is a tab of its own. **A question's own words are not here** either: what the catalog holds is the chrome, the two counts and the column headings. The Sales Channel a row names is read from `sales`, never re-coined, so there is only ever one Spanish word for a door sale |
