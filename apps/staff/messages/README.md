...
| `ticketAnswers` | One Ticket Sale's Tickets and the **Answer** each of them gives (#310), opened from a row of the Sales list. Its own namespace and not part of `sales`, because the surface is one Ticket's replies rather than anything about the sale — the sale is only how staff reach a Ticket. **An Answer's own words are not here** either: a reply is somebody's own words about themselves, drawn as written in both languages, and what the catalog holds is the chrome, the reasons the edit window is shut, and the ways an answer can be refused |
| `outstandingAnswers` | An Event's **Outstanding Answers** (#313): which Tickets still owe required Ticket Questions an Answer. Its own namespace and not part of `ticketAnswers`, because that surface is one sale's replies while this one is the whole Event's debt — one is opened from a sales row, the other is a tab of its own. **A question's own words are not here** either: what the catalog holds is the chrome, the two counts and the column headings. The Sales Channel a row names is read from `sales`, never re-coined, so there is only ever one Spanish word for a door sale |
