...
| `customerDossier` | The **Customer Dossier** ("Ficha del cliente", #635): everything one Event knows about one Customer — who they are, each Sale with its Tickets, Holders, Answers and surroundings, and the Tickets they hold on somebody else's Sale — plus the link that opens it from a row of the Sales list and of the Holder List. Its own namespace and not part of `sales` or `outstandingAnswers`, because its subject is one Customer, which both lists only lead to. **An Answer's own words are not here** either, for the reason `ticketAnswers` gives |
