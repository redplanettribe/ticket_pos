 ...
 * set a cookie, and only a route handler can do both. The Member never sees it:
 * it mints one pending sign-in, remembers it in a cookie only the callback will
 * ever be sent, and hands the browser to Google.
 *
 ...
 * wrong host. Google's URL is absolute and resolves regardless; a Staff path is
 * resolved by the browser against the URL it actually asked for.
 */
