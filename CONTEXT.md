...
Generated on the spot rather than queued, and so capped — at a number of Tickets, which is its own ceiling and not the Sales Export's cap on sales: nobody imports a roster, so there is no importer to be symmetric with. Filters matching more than the cap are refused with a message naming how many Tickets matched, because the filters are the lever. Every generated file leaves one log line naming who took it, from which Organization and Event, under which structural filters, and how many rows — never the search term, since it matches customer email addresses and a log aggregator is a wider audience than the database.
