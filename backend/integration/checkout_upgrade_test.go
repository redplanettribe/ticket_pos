...
// rides the BEGIN-CHECKOUT body alone — the one request the buyer makes with the
// prompt in front of them — and is snapshotted onto the Payment there (migration
// 124), to be read back inside whichever transaction settles it: begin itself
// for a free cart, the Payment Provider's return leg for a paid one. The confirm
// body carries no election and is never asked for one. Nothing on the way in
// validates it. Eligibility is
...
// confirmUpgradedCheckout settles a Payment on the provider's return leg, for a
// checkout whose buyer elected an Upgrade at begin.
//
// IT RELAYS NO ELECTION, AND THAT IS THE POINT OF IT BEING SPELLED OUT HERE. The
// return leg is an ordinary confirm — the same body an un-upgraded checkout
// sends — because the election is durable on the Payment row and read under the
// lock this settlement already takes. Nothing the browser does or fails to do on
// the way back changes what the buyer chose.
func confirmUpgradedCheckout(t *testing.T, env *testEnv, clientTransactionID string) confirmCheckoutResult {
	t.Helper()
...
func TestSameBasketUpgradeBuysThePaidLineAlone(t *testing.T) {
	...
	confirmUpgradedCheckout(t, payphoneEnv, begun.ClientTransactionID)
...
// TestTheElectionSurvivesAReturnLegThatCarriesNothing pins the transport, and it
// is the failure mode #649 flagged and #650's column closed.
//
// The buyer ticks the prompt at begin-checkout, goes to the Payment Provider,
// and comes back on a redirect that knows nothing about any of it — an older
// client, a browser that dropped its state, a webview that lost the tab. The
// Upgrade still happens, because the election was written onto the Payment at
// begin and is read back under the lock that settles it. Were the election
// carried on the return body instead, this shape would silently keep both
// Tickets and quietly contradict what the buyer chose.
//
// IT IS NOT A DURABILITY RISK. Eligibility is re-evaluated inside the commit
// regardless (TestUpgradeEligibilityIsReEvaluatedAtCommit), so a remembered
// election can never surrender a Ticket the platform would not surrender now.
func TestTheElectionSurvivesAReturnLegThatCarriesNothing(t *testing.T) {
	env := setupTest(t)
	enableTicketAssignment(t)
	enableTicketAssignmentThroughPayPhone(t)
	sessionID := orgAdminSession(t, env)
	...
	begun := beginCheckoutAsOK(t, payphoneEnv, testOrgSlug, "lost-election-fest", ana,
		electing(mixedBasket("ana@example.com", freeID, paidID)))
	// And nowhere at all on the leg that settles the Payment.
	confirmCheckoutParams(t, payphoneEnv, begun.ClientTransactionID, payphoneReturnParams(begun.ClientTransactionID))

	assertLines(t, saleOf(t, env, sessionID, eventID, "ana@example.com"),
		map[string]int{"VIP": 1}, "an election the Payment row remembered")
	if got := soldCount(t, env, sessionID, eventID, freeID); got != 0 {
		t.Fatalf("the giveaway's sold_count = %d, want 0 — the free line was never bought", got)
	}
}

// --- where the two mechanisms meet ------------------------------------------
//
// The Upgrade has two halves and they share one checkbox: the free Ticket in
// THIS basket is never bought (#649), and the free Ticket on an EARLIER Sale is
// reversed inside the paid commit (#650). The two tests below are the crossing,
// and they were written when the halves were merged onto one branch — neither
// ticket could assert them alone.

// TestABasketBesideAnEarlierFreeSaleSurrendersNothing is the hazard #650 named
// and this is the assertion that it is inert.
//
// THE FEARED FAILURE. #650 counts the basket's free Tickets off the lines AS
// WRITTEN, and #649 drops the basket's free line BEFORE they are written. If
// those two met badly, the count would read zero on a basket that really held a
// free Ticket, the ambiguity guard would open, and one election would take the
// earlier Sale AS WELL AS the dropped line: the buyer would lose two Tickets
// having elected to lose one. ADR 0074 calls that the mirror of the worst thing
// this system can do.
//
// WHY IT CANNOT HAPPEN, and what this test really pins: both halves ask the same
// OffersUpgrade, "earlier + basket == 1". This shape is earlier=1 and basket=1,
// which is 2 — ambiguous — so NEITHER half fires. Nothing is surrendered at all,
// which is the honest answer to an ambiguous election and the only one that
// cannot cost somebody a Ticket.
//
// EVERY OUTCOME IS ASSERTED AND NOT JUST THE HEADLINE. The buyer still holds the
// earlier Sale's Ticket (not merely: the Sale is still active), both basket
// lines were bought, capacity says so, the payment succeeded, and nobody was
// written to — because a reversal that told somebody would at least be visible,
// and the one this test guards against would not be.
func TestABasketBesideAnEarlierFreeSaleSurrendersNothing(t *testing.T) {
	env := setupTest(t)
	enableTicketAssignment(t)
	enableTicketAssignmentThroughPayPhone(t)
	sessionID := orgAdminSession(t, env)
	eventID, freeID, paidID := publishUpgradeEvent(t, env, sessionID, "Crossing Fest", "crossing-fest")

	ana := buyerSession(t, env, "ana@example.com")
	earlierSaleID, _ := claimFreeTicket(t, env, "crossing-fest", "ana@example.com", "Ana", "Lopez", freeID, 1)
	earlierTickets := ticketIDsOfSale(t, env, earlierSaleID)
	if len(earlierTickets) != 1 {
		t.Fatalf("the earlier free claim minted %d Tickets, want 1", len(earlierTickets))
	}
	assertBuyerHolds(t, env, earlierTickets[0], "ana@example.com")
	// That claim's own Sale Confirmation is not what this test is about, and
	// leaving it captured would make the silence assertion read a mail it never
	// meant.
	env.email.Reset()

	// One free Ticket on file, one free Ticket in the cart, and the election
	// ticked anyway — the body a stale tab or a forged request would send.
	begun := beginCheckoutAsOK(t, payphoneEnv, testOrgSlug, "crossing-fest", ana,
		electing(mixedBasket("ana@example.com", freeID, paidID)))
	confirmUpgradedCheckout(t, payphoneEnv, begun.ClientTransactionID)

	// THE EARLIER SALE IS UNTOUCHED, and the Ticket on it is still the buyer's.
	// Holding is what the buyer would miss, so holding is what is asserted.
	earlier := readSaleProvenance(t, env, earlierSaleID)
	if earlier.Status != "active" {
		t.Fatalf("the earlier free Sale reads %q, want active — an ambiguous election surrenders nothing", earlier.Status)
	}
	if earlier.ReplacedBy.Valid || earlier.ReplacementFor.Valid {
		t.Fatalf("the earlier free Sale was linked as replaced (%v, %v); nothing replaced it",
			earlier.ReplacedBy, earlier.ReplacementFor)
	}
	assertBuyerHolds(t, env, earlierTickets[0], "ana@example.com")

	// AND THE BASKET KEPT BOTH LINES. The drop must not have fired either: one
	// half firing while the other stayed shut would be the same bug wearing the
	// other face.
	saleID := saleIDOfPayment(t, env, begun.ClientTransactionID)
	var bought bool
	for _, row := range eventSales(t, env, sessionID, eventID) {
		if row.ID == saleID {
			assertLines(t, row, map[string]int{"GA": 1, "VIP": 1}, "a basket beside an earlier free Sale")
			bought = true
		}
	}
	if !bought {
		t.Fatalf("the paid Sale %s is not on the Sales list", saleID)
	}
	if got := salesCountByEmail(t, env, eventID, "ana@example.com"); got != 2 {
		t.Fatalf("the buyer has %d active Sales, want 2 — both stand", got)
	}
	// Capacity is the third witness: two giveaways sold, the earlier claim and
	// the line that was never dropped.
	if got := soldCount(t, env, sessionID, eventID, freeID); got != 2 {
		t.Fatalf("the giveaway's sold_count = %d, want 2 — nothing was given back and nothing was dropped", got)
	}
	if got := soldCount(t, env, sessionID, eventID, paidID); got != 1 {
		t.Fatalf("VIP sold_count = %d, want 1", got)
	}
	assertNobodyWasToldAboutAReversal(t, env)
}

// TestAPaidOnlyBasketUpgradesOutOfTheEarlierFreeSale is the positive crossing,
// and the one shape #649 could not assert on its own: the election it plumbs
// through reaching the OTHER mechanism.
//
// A buyer holding a free Ticket puts nothing free in their cart, ticks the
// prompt, and pays. There is no line to drop — #649's half returns the basket
// untouched — so the earlier Sale is reversed inside the paid commit instead,
// and the buyer ends the transaction holding the Ticket they paid for and no
// other.
//
// THE SILENCE IS THE HALF THAT COULD ROT QUIETLY. A reversal that started
// sending mail would still pass every assertion about rows, and the buyer would
// learn from an inbox that something they elected was something they lost.
func TestAPaidOnlyBasketUpgradesOutOfTheEarlierFreeSale(t *testing.T) {
	env := setupTest(t)
	enableTicketAssignment(t)
	enableTicketAssignmentThroughPayPhone(t)
	sessionID := orgAdminSession(t, env)
	eventID, freeID, paidID := publishUpgradeEvent(t, env, sessionID, "Moving Up Fest", "moving-up-fest")

	ana := buyerSession(t, env, "ana@example.com")
	earlierSaleID, _ := claimFreeTicket(t, env, "moving-up-fest", "ana@example.com", "Ana", "Lopez", freeID, 1)
	earlierTickets := ticketIDsOfSale(t, env, earlierSaleID)
	if len(earlierTickets) != 1 {
		t.Fatalf("the earlier free claim minted %d Tickets, want 1", len(earlierTickets))
	}
	env.email.Reset()
	assertSurrenderable(t, surrenderableFreeTickets(t, env, "moving-up-fest", ana), 1, "before the paid basket")

	begun := beginCheckoutAsOK(t, payphoneEnv, testOrgSlug, "moving-up-fest", ana,
		electing(checkoutBody("ana@example.com", "Ana", "Lopez", cartLine(paidID, 1))))
	confirmUpgradedCheckout(t, payphoneEnv, begun.ClientTransactionID)

	paidSaleID := saleIDOfPayment(t, env, begun.ClientTransactionID)
	earlier := readSaleProvenance(t, env, earlierSaleID)
	if earlier.Status != "reversed" {
		t.Fatalf("the earlier free Sale reads %q, want reversed — the buyer moved up", earlier.Status)
	}
	if earlier.ReplacedBy.String != paidSaleID || earlier.ReplacementFor.String != "upgrade" {
		t.Fatalf("the earlier free Sale is linked as (%v, %v), want replaced by %s for an upgrade",
			earlier.ReplacedBy, earlier.ReplacementFor, paidSaleID)
	}

	// The buyer holds the Ticket they paid for, and exactly one Sale stands.
	paidTickets := ticketIDsOfSale(t, env, paidSaleID)
	if len(paidTickets) != 1 {
		t.Fatalf("the paid Sale minted %d Tickets, want 1", len(paidTickets))
	}
	assertBuyerHolds(t, env, paidTickets[0], "ana@example.com")
	assertLines(t, saleOf(t, env, sessionID, eventID, "ana@example.com"),
		map[string]int{"VIP": 1}, "after moving up out of an earlier free Sale")
	if got := salesCountByEmail(t, env, eventID, "ana@example.com"); got != 1 {
		t.Fatalf("the buyer has %d active Sales, want 1 — the free one was reversed", got)
	}
	// The surrendered Ticket returns to the Event.
	if got := soldCount(t, env, sessionID, eventID, freeID); got != 0 {
		t.Fatalf("the giveaway's sold_count = %d, want 0 — the surrendered Ticket goes back", got)
	}
	if got := soldCount(t, env, sessionID, eventID, paidID); got != 1 {
		t.Fatalf("VIP sold_count = %d, want 1", got)
	}

	// THE SILENCE, and the one mail that is owed: the paid Sale's Confirmation.
	assertNobodyWasToldAboutAReversal(t, env)
	if confirmations := env.email.Confirmations(); len(confirmations) != 1 {
