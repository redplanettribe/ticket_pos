...
// reverseSale reverses a recorded Ticket Sale. Programmatic reversal of an
// Online Sale does not exist yet (ADR 0012), but the balance must already
// respect the status a reversal will set.
func reverseSale(t *testing.T, env *testEnv, confirmationRef string) {
	t.Helper()
	result, err := env.db.Exec(`UPDATE ticket_sales SET status = 'reversed' WHERE confirmation_ref = $1`, confirmationRef)
	if err != nil {
