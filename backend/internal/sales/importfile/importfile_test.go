...
func TestParseCSVHeadersAndRows(t *testing.T) {
	csv := "customer_email,customer_name,ticket_type,quantity,payment_method,sold_at,amount\n" +
		"ana@example.com,Ana,GA,2,cash,2026-07-01T10:00:00Z,10.50\n" +
		"\n" + // blank row skipped
		"bob@example.com,Bob,VIP Pass,1,transfer,2026-07-02,\n"
	...
	if rows[0].CustomerEmail != "ana@example.com" || rows[0].TicketType != "GA" || rows[0].Amount != "10.50" {
		t.Fatalf("row0 = %+v", rows[0])
	}
	// encoding/csv collapses blank lines, so bob is the second data record.
	if rows[1].CustomerEmail != "bob@example.com" || rows[1].TicketType != "VIP Pass" {
...
func TestParseRowLimit(t *testing.T) {
	...
	b.WriteString("customer_email,customer_name,ticket_type,quantity,payment_method,sold_at\n")
	for i := 0; i < MaxRows+1; i++ {
		b.WriteString("ana@example.com,Ana,GA,1,cash,2026-07-01T10:00:00Z\n")
	}
...
func TestValidateReportsAllErrorsAtOnce(t *testing.T) {
	...
	rows := []RawRow{
		{Line: 2, CustomerEmail: "not-an-email", CustomerName: "", TicketType: "Nope", Quantity: "0", PaymentMethod: "bitcoin", SoldAt: "2030-01-01T00:00:00Z"},
	}
	...
	for _, field := range []string{"customer_email", "customer_name", "ticket_type", "quantity", "payment_method", "sold_at"} {
		if !got[field] {
...
func TestValidateMatchesAndCapacityImpact(t *testing.T) {
	...
	rows := []RawRow{
		...
		{Line: 3, CustomerEmail: "bob@example.com", CustomerName: "Bob", TicketType: "GA", Quantity: "3", PaymentMethod: "transfer", SoldAt: "2026-07-02", Amount: "0"},
	}
	res := Validate(ValidateInput{Rows: rows, Types: sampleTypes(), Now: now, Location: time.UTC})
	if res.ValidRows != 2 {
...
func TestValidateMatchesByHiddenID(t *testing.T) {
	now := time.Date(2026, 7, 7, 12, 0, 0, 0, time.UTC)
	rows := []RawRow{
		{Line: 2, CustomerEmail: "ana@example.com", CustomerName: "Ana", TicketType: "typo name", TicketTypeID: "tt-vip", Quantity: "1", PaymentMethod: "cash", SoldAt: "2026-07-01T10:00:00Z"},
	}
	res := Validate(ValidateInput{Rows: rows, Types: sampleTypes(), Now: now, Location: time.UTC})
...
func TestValidateAmountDecimalToCents(t *testing.T) {
	now := time.Date(2026, 7, 7, 12, 0, 0, 0, time.UTC)
	rows := []RawRow{
		{Line: 2, CustomerEmail: "ana@example.com", CustomerName: "Ana", TicketType: "GA", Quantity: "1", PaymentMethod: "cash", SoldAt: "2026-07-01T10:00:00Z", Amount: "10.50"},
	}
	res := Validate(ValidateInput{Rows: rows, Types: sampleTypes(), Now: now, Location: time.UTC})
...
func TestTemplateRoundTrip(t *testing.T) {
	...
	wantHeaders := []string{"customer_email", "customer_name", "ticket_type", "quantity", "payment_method", "sold_at", "amount", "ticket_type_id"}
