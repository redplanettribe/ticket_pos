...
type beginCheckoutBody struct {
	...
	// The guest checkout this ticket builds always shows all three and therefore
	// always sends all three; a signed-in Customer is shown only what they have
	// not answered (#254), and reading their absent boxes as refusals would turn
	// their purchase into a marketing opt-out.
	//
	// PolicyAcceptance is REQUIRED to be present and true. It is not a field error
	// but a domain refusal — POLICY_ACCEPTANCE_REQUIRED from the service — because
	// what is wrong is not the shape of the request but that the platform may not
	// act on it (ADR 0035, consent.ErrPolicyAcceptanceRequired).
