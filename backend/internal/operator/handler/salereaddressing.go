...
// WithdrawReAddressing ends the pending Sale Re-addressing on one Ticket Sale.
//
// @Summary      Withdraw the pending re-addressing of an Online Sale
...
// @Security     BearerAuth
// @Param        confirmationRef  path  string  true  "Sale Confirmation reference (case-insensitive)"
// @Success      200  {object}  openapi.EnvelopeOperatorSaleReAddressing
// @Failure      401  {object}  platform.Envelope
// @Failure      403  {object}  platform.Envelope
// @Failure      404  {object}  platform.Envelope
// @Failure      409  {object}  platform.Envelope
// @Router       /api/v1/operator/sales/{confirmationRef}/re-address [delete]
func (h *Handler) WithdrawReAddressing(w http.ResponseWriter, r *http.Request) {
	reqID := platform.RequestID(r.Context())
	withdrawn, err := h.svc.WithdrawReAddressing(r.Context(), strings.TrimSpace(r.PathValue("confirmationRef")))
	if err != nil {
		_ = platform.WriteDomainError(w, reqID, err)
		return
	}
	_ = platform.WriteSuccess(w, reqID, http.StatusOK, withdrawn)
}
