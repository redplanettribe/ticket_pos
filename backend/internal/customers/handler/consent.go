...
type consentSubmitBody struct {
	...
	// AdulthoodDeclaration is the 18+ box (#586, ADR 0069): the affirmation that
	...
	// PresentedLocale is the language of the LEGAL TEXT this step rendered — the
...
func (b consentSubmitBody) withdrawalOnly() bool {
	...
	// And a submission mentioning the Adulthood Declaration is a sign-in
	// submission too (#586). It is an affirmation on a required box, and there
	// is no channel on which a declaration is withdrawn — so no reading of this
	// field may steer a submission onto the withdrawal branch, where the
	// required boxes are not judged at all.
	if b.AdulthoodDeclaration != nil {
		return false
	}
...
func (h *Handler) SubmitConsent(w http.ResponseWriter, r *http.Request) {
	...
	// The language the notice was actually served in, off the body and dropped
	// when it names one this platform does not serve — see the field's note. It
	// is attached only on this branch: the withdrawal above showed nothing.
	if presented, ok := platform.ParseLocale(body.PresentedLocale); ok {
		evidence.PresentedLocale = presented
	}
	...
	outcome, err := h.svc.SubmitConsent(r.Context(), service.ConsentSubmission{
		...
		// Absent is false here too, and false where owed is refused before any
		// write (#586).
		AdulthoodDeclaration: body.AdulthoodDeclaration != nil && *body.AdulthoodDeclaration,
