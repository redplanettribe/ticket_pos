...
func (h *Handler) ExportHolderList(w http.ResponseWriter, r *http.Request) {
	...
	started := false
	err := h.svc.ExportHolderList(r.Context(), actorFromRequest(r), eventID, params, func(filename string) io.Writer {
		started = true
	...
	if errors.As(err, &domainErr) && domainErr.Code() == "HOLDER_EXPORT_BUSY" {
		// Capacity, not the request: say when trying again is worth it.
