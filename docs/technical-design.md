...
- The `request` log line is written for every request, at ERROR for a 5xx and at WARN with `aborted=true` when a streamed download aborts after its first byte (`panic(http.ErrAbortHandler)`), carrying the status already sent.
...
It is answered as a well-formed id that names nothing is: `404` with that resource's own code (`EVENT_NOT_FOUND`, `TICKET_TYPE_NOT_FOUND`, ...), or the empty list on the few reads whose "not found" is an empty list.
...
The platform issues electronic facturas to Ecuador's SRI for the one taxable service it sells — the Platform Fee with its Fee IVA — by hand, from the Operator Dashboard (#450, ADR 0059; vocabulary in `CONTEXT.md` *Tax invoicing*), and, since #471 (ADR 0060), for the tickets of a **House Organization**: an Organization the platform's own entity runs, whose paid Online Sales are the platform's own sale and owe the buyer a **Sale Invoice**, and whose reversals owe a **Credit Note**.
The platform is the **sole Issuer**: one RUC, one certificate, one set of authorizations per environment. No Organization is an emisor and no Member or Integration Partner route exists; the one Customer route is the buyer's read of their own Sale's documents.

### The `invoicing` module

`internal/invoicing/` follows the domain / repository / service / handler split and is served on the operator namespace under `/api/v1/operator/invoicing/*`, behind the operator allowlist middleware and nothing else — plus one internal endpoint the scheduler ticks and two Customer routes (below).
It owns its own tables. It reads the sales side only where a Sale-side document needs it: a Sale Invoice and a Credit Note carry the Ticket Sale they are about (`ticket_sale_id`, no `ON DELETE`), the operator surfaces read the Sale Confirmation reference beside the row, and delivery reads the Event's name and the Sale and Mail Locales for the buyer's mail. No Payout or Organization is linked; the House designation is the identity module's fact, and the sales module asks it at checkout.
...
### The sales → invoicing seam

The sales module never learns what a factura is; it tells the invoicing module three things through a `SaleInvoicer` interface it defines (`sales/service`), and the invoicing service implements (ADR 0060):

| Moment | Call | Inside the sale's transaction? |
|--------|------|-------------------------------|
| A paid Online Sale of a House Event is recorded | `OwePaidOnlineSale` — the whole document minus everything that needs an Issuer, written in state `owed` | Yes: if the row cannot be written, the sale is not recorded |
| That transaction has committed | `KickSaleInvoiceDrainer` — one immediate round over that Sale's documents, in the background | No: fire-and-forget, detached from the request's context |
| A Ticket Sale is marked reversed, by any route | `SettleReversedSale` — the Sale Invoice's fate decided under `FOR UPDATE`: never sent → `withdrawn`; authorized → a Credit Note owed; signed and undecided → a Credit Note owed that waits | Yes: the reversal and its paperwork commit together, and a reversal is never refused or delayed by the state of its paperwork |

Everything a document needs is **copied at owing time** — the Recipient is the Sale's buyer as transacted, the lines are the Ticket Sale Lines at the price paid with IVA backed out at the platform's one rate, the payment method is derived from the provider — and nothing is re-read from the Customer or the catalog afterwards. A House sale's Sale Confirmation says a factura will follow, in the Sale's Locale. Nothing here can refuse for the Issuer's sake: no Issuer, a test Issuer, an expired certificate are the Drainer's to find and the operator's to fix.

### The Sale Invoice Drainer

`POST /api/v1/internal/sale-invoices/drain` is the Reversal Reconciler's shape, reused: an authenticated internal endpoint (Cloud Scheduler with an OIDC token; a human can curl it), a queue that is a column on the feature's own table (`invoicing_invoices.next_attempt_at`), one document claimed at a time under a lease (`UPDATE … WHERE id = (SELECT … FOR UPDATE SKIP LOCKED LIMIT 1)`, the lease being `next_attempt_at` moved five minutes out in the claiming statement), and a run bounded by a budget of its own (60 s, strictly inside the scheduler's 120 s attempt deadline, inside Cloud Run's 300 s; a test reads the terraform variable rather than mirroring it).
The scheduler job (`terraform/modules/ticket-pos/sale_invoice_drainer.tf`) ships **paused by default** (`sale_invoice_drainer_enabled = false`) and is unpaused per environment once the Issuer is real; until then the post-commit kick and a curl are the only rounds.

A round on an **owed** document signs it — inside one transaction that allocates the secuencial, builds the factura from the row's own snapshot under the Issuer's *current* environment, and signs it — then submits and polls exactly as the manual issue does. Everything that can refuse does so before that transaction, so a document that cannot be signed (no Issuer, no certificate, an expired one, an Issuer the schema refuses) is parked `needs_attention` with a `PLATFORM`-typed message saying why and **no number consumed**, and retried hourly so the operator's fix heals it on the next tick. A document the authority holds is **polled, never resubmitted**; only one every submit failed to reach the authority is sent again under the same clave. The ladder is 1, 5, 15 minutes then hourly, measured from the signing instant, for transport failures and "en procesamiento" alike; a definite refusal parks the document and stops the ladder (Check status and Resend are the remedies, and work on a Sale Invoice as on a manual document); 24 hours without a definite answer parks it too, still polled hourly, so a late AUTORIZADO heals it with nobody pressing anything.
A Credit Note rides the same machine: claimed only once the factura it credits is terminal (decided at claim time, by reading the factura's state, so nothing has to remember the Credit Note); signed under codDoc 04 from the factura's number and date with the reversal route as its `motivo`; or, when the factura died unauthorized (withdrawn, annulled), withdrawn with it, unsigned and unsent.

**A claim is a lease, not a lock.** A reversal may withdraw an owed document under the round that holds it, and an operator may mark a signed one annulled; every write a round makes is therefore guarded on the status it read the row in (`Reschedule`, `ApplyOutcome` report "nothing written" on zero rows), a round that finds its row moved writes nothing more and reports the row as it stands, and the authority's late answer lives in the attempts ledger and nowhere else. A dead document is never resurrected onto the queue by a round that started before it died.

The drain response is counts and states only — claimed, authorized, pending, needs_attention, delivered, withdrawn, failed, and the standing count per state — and so are the log lines: never a buyer.

### Delivery

Delivery is the Drainer's last step and asks the authority nothing. An authorized Sale-side document is mailed to the buyer in the same round that saw it authorized — one message in the Sale's Locale through the transactional sender, the signed XML attached under `<clave>.xml`, the Event and the Sale Confirmation reference in the words, and a link to the Sale's card in the Customer Area — and `delivered_at` is recorded only once the sender took it (`MarkDelivered`, guarded on `delivered_at IS NULL`).
The authorization **keeps the round's lease** rather than releasing it (`ApplyOutcome` leaves `next_attempt_at` alone on an authorized Sale-side document), so a second round cannot claim the document between the authorization and the mail and send it twice; a sender that refused reschedules delivery alone on the ladder, and an authorized document without a `delivered_at` is due work exactly as an owed one is. A document the operator's Check status or Resend heals holds no lease, so those actions make it due themselves (`DueForDelivery`, only when nothing was due) and the next round mails it.
The Credit Note rides delivery unchanged; the only place the kind matters is the copy's choice of words.

### The buyer's and the operator's routes

The Customer Area reads a Sale's documents through `GET /api/v1/customer/ticket-sales/{ticketSaleId}/tax-documents` and downloads one through `…/tax-documents/{id}/xml`, behind the Customer Session gate and nothing more, scoped to the Sale the session owns (a Confirmation Link session is narrowed to the one Sale it names). The buyer is told two things and never a third — `authorized` with a download, or `on_its_way` for owed, pending and needs_attention alike — no message, no attempt, no number travels, and a withdrawn document is never shown.
The operator side adds `GET /operator/invoicing/needs-attention` and `…/needs-attention/count` (the Operator Dashboard's queue, longest waiting first by `attention_since`, with the authority's or the platform's last message beside each row), `POST /operator/invoicing/invoices/{id}/annul` — **Mark annulled**, the operator's record of a manual portal act the SRI offers no web service for, allowed only from `pending` or `needs_attention` on a signed document, guarded on those states in the write so a press racing a late AUTORIZADO annuls nothing, irreversible, and what takes the document off the queue and out of the Drainer's claim set in one statement — a `kind` filter on the invoices list, and the Sale lookup's walk from a Sale to its documents. Check status and Resend refuse an authorized document (`INVOICE_ALREADY_AUTHORIZED`), an annulled one (`INVOICE_ANNULLED`), a withdrawn one (`INVOICE_WITHDRAWN`) and one still owed (`INVOICE_NOT_ISSUED`), each a 409 in its own words.
...
Migration 097 lands the House designation on `organizations` (#472): `house_designated_by` (the operator's email, ADR 0015) and `house_designated_at`, both set or both null under one CHECK — the pair *is* the flag, and there is no history to keep, since undesignation affects future sales only and withdraws nothing already owed. The USD-only rule is the identity service's, not the schema's.
Migration 098 lands the Sale Invoice and the Credit Note (#473) on the same tables, additively: `kind` (`manual` / `sale` / `credit_note`), `ticket_sale_id`, `credits_invoice_id` and `reversal_reason` (the five reversal routes, as the Sales Export spells them), `iva_rate` (stated once on the document so a reader never infers a platform rule from its lines), `delivered_at` and `next_attempt_at` (the Drainer's queue, NULL when nothing is due). A document now **exists before it is issued**: the seven issue-time columns (issuer, environment, emission date and instant, signer, snapshot, signed XML) become nullable *together* under one CHECK — signed and has all of them, or owed and has none — and the Ecuador detail row is absent until signing, which is what "no sequence number consumed" means. Four states join the four of 096: `owed`, `needs_attention`, `withdrawn` (never sent, never will be) and `annulled` (the operator's record of a manual portal act on a signed, unauthorized document).
Migration 099 lands the two facts the queue and Mark annulled need (#477): `attention_since` — set the instant the row enters `needs_attention`, kept while it stays, cleared when it leaves, held to the state by a CHECK, so the queue's "since when" is the parking and never the last poll — and `annulled_by` / `annulled_at`, set exactly when the row is annulled, the number, clave and signed XML kept because the annulment is a fact about a document that existed.
