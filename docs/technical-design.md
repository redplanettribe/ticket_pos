...
- The line's status and level follow what actually happened.
  ...
  A recovered panic's 500 is written by the recovery middleware, not through `WriteDomainError`, so its line carries neither `error_code` nor `error_class`.
...
- A client that gives up is not a server error, but a client leaving never downgrades what happened.
  ...
  The second is when the handler reported no error, wrote nothing, and the client had gone by the time it returned; nothing was written at all, and the line carries neither `error_code` nor `error_class`.
