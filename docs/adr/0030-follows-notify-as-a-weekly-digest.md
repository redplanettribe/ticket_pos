...
- **Digests send from a sending domain separate from transactional mail.** One-time Passcodes,
  ...
  subdomain, provider domain and DNS records. **Amended 2026-08-05:** production shares the
  transactional domain, because the Resend plan verifies only one — see *The domain split under a
  one-domain plan* below. The separation remains the design; sharing it is a declared exception.
...
- **The domain split is not about Follows at all.** It is the highest-risk detail in the feature.
  ...
  coupling entirely.
- **The domain split under a one-domain plan (amended 2026-08-05).** The reasoning above survives
  contact with the bill and the deployment does not: Resend's free tier verifies exactly ONE
  domain, so `digest.multiticketing.com` cannot be added at all. That turns the decision from "two
  domains or one" into "one domain or no Follow Digests", and the second is worse — the feature
  ships dark, while the harm avoided is a reputation effect that needs a complaint volume the same
  plan's sending limits will not reach. Production therefore sets
  `DIGEST_EMAIL_ALLOW_SHARED_DOMAIN`, and the API sends Digests from the transactional domain.

  Two things keep this an exception rather than a quiet reversal. The API still **refuses** a Digest
  on the transactional domain by default, so the sharing is never inferred from the two domains
  matching — that shape is also what a typo in `DIGEST_EMAIL_FROM` looks like, and it should keep
  failing. And the flag is one boolean to unset the day a second domain is affordable, with the
  Terraform for the separate domain left in place and its DNS checklist still printed.

  What is genuinely given up: a spike in Digest complaints can now degrade delivery of One-time
  Passcodes, which is the login path. If OTP deliverability drops, the Digest is the first thing to
  suspect, and turning it off is a flag rather than a deploy.
