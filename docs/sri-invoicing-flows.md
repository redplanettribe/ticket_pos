...
| 59 | Certificate expiry | **Partial** | `NotAfter` shown on the Issuer page (`B/invoicing/service/service.go:294`); expired → park (`drainer.go:621`) | No early warning, mail or dashboard alert; a lapse is discovered when documents start parking. Rotation = re-upload. → U9 |
...
| U9 | No certificate-expiry early warning | 59 | Every House sale parks `needs_attention` from the minute the `.p12` expires until someone re-uploads; the 24 h SRI window and the "immediate transmission" rule are both running. |
